...
# Borrowed from 'INM-707 Deep Reinforcement Learning Exercise' (Class of 2021).

# %% Define data structure to hold information about actions
...
class Dungeon:
    ...
    def display(self):
        env_with_agent = self.dungeon.copy()
        env_with_agent[self.position_agent[0], self.position_agent[1]] = 4
    ...
    def calculate_observations(self):
        ...
        surroundings = dungeon_padded[
                       self.position_agent[0] + 1 - 2: self.position_agent[0] + 1 + 3,
                       self.position_agent[1] + 1 - 2: self.position_agent[1] + 1 + 3]

        obs = {'relative_coordinates': relative_coordinates,
               'surroundings': surroundings}

        relative_coordinates = relative_coordinates.flatten()
        surroundings = surroundings.flatten()
        observation_concat = np.concatenate([relative_coordinates, surroundings])

        return observation_concat
    ...
    def reset(self):
        ...
        # Calculate observations
        observation_concat = self.calculate_observations()

        return observation_concat


# %% Modifying the Ice Dungeon for Partial Observability
...
class IceDungeonPO(Dungeon):
    ...
    def __init__(self, N):
        ...
        # In order to explicitly show that the way you represent states doesn't matter,
    ...
    def step(self, action):
        ...
        if done:
            ...
            prob_of_slipping = [0.1, 0.1, 0.1, 0.1, 0.6]

            index_action = np.argmax(np.random.multinomial(1, prob_of_slipping))
