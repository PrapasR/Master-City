...
# Adapted from 'INM707 Deep Reinforcement' Exercise (Class of 2021).

# %% Warehouse Environment

'''
Rules:
    - Starting environment with random placing obstacles, a parcel and a destination.
    - Randomly placing the agent around the edge of environment when reset the environment.
    - There are 3 types of obstacles, which are stationary obstacles (N/2), humans (N/10) and a box.
    - All humans will move within the boundary 2x2 (to the left of their starting position), humans will only move to
      the free space. Human have 60% of not moving, and the rest will be shared among the remaining valid actions.
    - The agent starts at the charging station, which located randomly at the border.
    - The agent cannot move when crash to the stationary obstacles.
    - The box is moved toward free a space, when the gent accidentally torch it.
    - The agent can push the box if there is a free space behind it.
